...
import org.eri.model.X_ER_ProductionLineAttribute;

public class CreateMasterProductionPlanBackup extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		String strSQL = "select ad_org_id, C_OrderLine_ID, M_Product_ID, qtyEntered, " +
						"(select coalesce(sum(qty), 0) from er_masterproductionline where c_orderline_id = c_orderline.c_orderline_id) as qtyplanning " +
						"from c_orderline where c_orderline_id in (SELECT T_Selection_ID From T_Selection WHERE AD_PInstance_ID = " + p_Instance_ID + ") order by ad_org_id, C_order_id";
