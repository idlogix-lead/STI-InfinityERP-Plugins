...
public class AddStrobelProduction extends SvrProcess{
	...
	protected void prepare() {
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				;
			else if (name.equals("ER_StrobelHdr_ID"))
				StrobelHdrID = para[i].getParameterAsInt();			
...
//		System.err.println("I am nt here!");
		...
		int rows = 0; 
		String resultMsg ="";
		String id=""; int hdrid=StrobelHdrID;
		MStrobelHdr StrobelHdr= new MStrobelHdr(Env.getCtx(), StrobelHdrID, null);
		...
		try
		{
			...
			while (rs.next ())		//	Order
			{
				String strSQLbal = "select * from er_strobel_balance_size_v where balanceqty>0 and M_Production_ID=" + rs.getInt("M_Production_ID"); 
				...
				rows =rows+1;				
				MStrobel pline = new MStrobel(Env.getCtx(), 0, null);
...
//				StrobelHdr.set_Value("Processed",'Y');
				StrobelHdr.save();	
