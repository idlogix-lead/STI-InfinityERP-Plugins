...
public class AddCuttingProduction extends SvrProcess{
	...
	protected void prepare() {
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				;
			else if (name.equals("ER_CuttingHdr_ID"))
				cuttingHdrID = para[i].getParameterAsInt();			
...
//		System.err.println("I am nt here!");
		...
		int rows = 0; 
		String resultMsg ="";
		String id=""; int hdrid=cuttingHdrID;
		MCuttingHdr cuttingHdr= new MCuttingHdr(Env.getCtx(), cuttingHdrID, null);
		...
		try
		{
			...
			while (rs.next ())		//	Order
			{
				String strSQLbal = "select * from er_cutting_balance_size_v where balanceqty>0 and M_Production_ID=" + rs.getInt("M_Production_ID"); 
				...
				rows =rows+1;				
				MCutting pline = new MCutting(Env.getCtx(), 0, null);
				...
				pline.set_ValueOfColumn("ER_CuttingHdr_ID", hdrid);
...
//				cuttingHdr.set_Value("Processed",'Y');
				cuttingHdr.save();	
