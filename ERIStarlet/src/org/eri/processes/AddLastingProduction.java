...
public class AddLastingProduction extends SvrProcess{
	...
	protected void prepare() {
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				;
			else if (name.equals("ER_LastingHdr_ID"))
				LastingHdrID = para[i].getParameterAsInt();			
...
//		System.err.println("I am nt here!");
		...
		int rows = 0; 
		String resultMsg ="";
		String id=""; int hdrid=LastingHdrID;
		MLastingHdr LastingHdr= new MLastingHdr(Env.getCtx(), LastingHdrID, null);
...
//				LastingHdr.set_Value("Processed",'Y');
				LastingHdr.save();	
