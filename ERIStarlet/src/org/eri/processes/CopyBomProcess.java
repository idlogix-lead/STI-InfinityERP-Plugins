...
public class CopyBomProcess extends SvrProcess {
...
//		}
		...
		for (MParentBOM l :lines) {
			target = new MParentBOM(getCtx(), 0, get_TrxName());
			MParentBOM.copyValues(l, target);
			target.setM_Parent_Product_ID(copyTo);
			target.saveEx(get_TrxName());
			count++;
		}
